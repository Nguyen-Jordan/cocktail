					...
					<?php
					 ...
					 if ( is_active_sidebar( 'cocktail_footer_options' ) ) :
						...
						<a title="<?php echo esc_attr( get_bloginfo( 'name', 'display' ) ); ?>" target="_blank" href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php echo get_bloginfo( 'name', 'display' ); ?></a> | 
									...
									<?php  date_i18n(__('Y','cocktail')) ; ?> <a title="<?php echo esc_attr__( 'WordPress', 'cocktail' );?>" target="_blank" href="<?php echo esc_url( 'https://wordpress.org' );?>"><?php esc_html_e('WordPress','cocktail'); ?></a> | <?php echo '&copy; ' . esc_html__('Copyright All right reserved ','cocktail'); ?>
						<?php
			...
			<?php
				...
				if($disable_scroll == 0):?>
					<button class="go-to-top" type="button">
						<span class="screen-reader-text"><?php esc_html_e('Go to top','cocktail');?></span>
