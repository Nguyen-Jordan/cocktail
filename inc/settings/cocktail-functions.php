...
function cocktail_scripts() {
	...
	wp_enqueue_script('cocktail-skip-link-focus-fix', get_template_directory_uri().'/js/skip-link-focus-fix.js', array('jquery'), false, true);
	...
	wp_register_style( 'cocktail-google-fonts', wptt_get_webfont_url('https://fonts.googleapis.com/css?family='.$cocktail_googlefonts .':400,400i,700'));
			...
			<div class="custom-header-wrap">
				<img src="<?php header_image(); ?>" class="header-image" width="<?php echo esc_attr(get_custom_header()->width);?>" height="<?php echo esc_attr(get_custom_header()->height);?>" alt="<?php echo esc_attr(get_bloginfo('name', 'display'));?>">
					<?php if($cocktail_settings['cocktail_header_image_title'] !=''){ ?>
						<h2 class="header-image-title"><a title="<?php echo esc_attr($cocktail_settings['cocktail_header_image_title']);?>" href="<?php echo esc_url($cocktail_settings['cocktail_header_image_link']);?>"><?php echo esc_html($cocktail_settings['cocktail_header_image_title']);?></a></h2>
						<?php } ?>
			</div>
